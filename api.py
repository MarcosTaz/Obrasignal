...
def _latest_decisions(conn, account_id, external_ids):
    ...
    import json
    for row in rows:
        item = dict(row); item["features"] = json.loads(item.pop("features_json") or "{}")
...
def profile():
    ...
    current = load_profile(identity.account_id); merged = dict(current)
...
def alerts():
    ...
    where = "WHERE e.account_id=? AND e.event_type='new_high_match'"; params = [identity.account_id]
    ...
    decisions = _latest_decisions(c, identity.account_id, [r["external_id"] for r in rows]); items = []
    for r in rows:
