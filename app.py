...
def db():
 ...
 if 'published_at' not in cols: c.execute('ALTER TABLE tenders ADD COLUMN published_at TEXT')
 c.commit(); return c
...
def lang_value(v):
 ...
 if isinstance(v,dict):
  for key in ['pt','pt-PT','por','en','en-GB','eng']:
...
def flat(v):
 if isinstance(v,list):
  vals=[]
  for x in v: vals.append(lang_value(pick(x,'label','name','value','code') or x) if isinstance(x,dict) else lang_value(x))
...
def parse_date(v):
 ...
 try:return datetime.fromisoformat(s).astimezone(timezone.utc).isoformat()
  ...
  for fmt in ('%Y-%m-%d','%Y%m%d'):
   try:return datetime.strptime(s[:10],fmt).replace(tzinfo=timezone.utc).isoformat()
...
def fetch_ted():
 ...
 for _ in range(TED_MAX_PAGES):
  ...
  r=requests.post(TED_URL,json=body,timeout=45); r.raise_for_status(); data=r.json(); batch=data.get('notices') or data.get('results') or data.get('content') or []; rows.extend(batch); token=data.get('iterationNextToken') or data.get('nextToken') or data.get('nextIterationToken')
...
def save(rows):
 ...
 for x in rows:
  ...
  exists=c.execute('SELECT id FROM tenders WHERE source=? AND external_id=?',(x['source'],x['external_id'])).fetchone(); sc=score(x['title'],x['description'],x['cpv'])
  if exists:c.execute('''UPDATE tenders SET title=?,description=?,buyer=?,country=?,cpv=?,value=?,deadline=?,publication_date=?,published_at=?,url=?,score=?,last_seen=? WHERE id=?''',(x['title'],x['description'],x['buyer'],x['country'],x['cpv'],x['value'],x['deadline'],x['publication_date'],x['published_at'],x['url'],sc,now,exists['id']))
  else:c.execute('''INSERT INTO tenders(source,external_id,title,description,buyer,country,cpv,value,deadline,publication_date,published_at,url,score,first_seen,last_seen) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(x['source'],x['external_id'],x['title'],x['description'],x['buyer'],x['country'],x['cpv'],x['value'],x['deadline'],x['publication_date'],x['published_at'],x['url'],sc,now,now)); new_items+=1
...
def sync():
 ...
 new_items=save(rows); c.execute('INSERT INTO sync_runs(started_at,finished_at,found,new_items) VALUES(?,?,?,?)',(started,datetime.now(timezone.utc).isoformat(),len(rows),new_items)); c.commit(); return len(rows),new_items
...
def index():
 ...
 rows=c.execute(sql+' ORDER BY score DESC, published_at DESC, first_seen DESC LIMIT 500',args).fetchall(); total=c.execute('SELECT COUNT(*) FROM tenders').fetchone()[0]; cutoff=(datetime.now(timezone.utc)-timedelta(hours=24)).isoformat(); new24=c.execute('SELECT COUNT(*) FROM tenders WHERE published_at>=?',(cutoff,)).fetchone()[0]; high=c.execute('SELECT COUNT(*) FROM tenders WHERE score>=70').fetchone()[0]; last=c.execute('SELECT MAX(finished_at) FROM sync_runs').fetchone()[0] or '—'; return render_template_string(HTML,tenders=rows,stats={'total':total,'new24':new24,'high':high,'last':last},q=q,country=country,minscore=ms)
...
@APP.get('/api/tenders')
def api():
 c=db(); return jsonify([dict(x) for x in c.execute('SELECT * FROM tenders ORDER BY score DESC,published_at DESC,first_seen DESC LIMIT 500').fetchall()])
def worker():
 while True:
  try:sync()
