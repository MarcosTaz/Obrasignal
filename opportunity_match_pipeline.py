...
def _lot_from_row(row):
    return {
        ...
        "title": row.get("title"), "description": row.get("description"), "buyer": row.get("buyer"),
        "country": row.get("country"), "cpv": row.get("cpv"), "value": row.get("value"),
        "value_numeric": row.get("value_numeric"), "deadline": row.get("deadline"),
...
def _hard_capability_checks(lot, capability):
    ...
    if amount is not None:
        if capability.get("min_value") is not None and amount < float(capability["min_value"]): blockers.append("valor abaixo do mínimo da empresa")
        if capability.get("max_value") is not None and amount > float(capability["max_value"]): blockers.append("valor acima do máximo da empresa")
    ...
    if procedure and procedure in excluded: blockers.append(f"procedimento excluído: {procedure}")
    ...
    if cpv and prefixes:
        ...
        if codes and not any(any(code.startswith(prefix) for prefix in prefixes) for code in codes): blockers.append("CPV fora das famílias aceites pela empresa")
    text_folded = " ".join(str(v) for v in (lot.get("title"), lot.get("description")) if v).casefold()
    for exclusion in capability.get("hard_exclusions", []):
        term = str(exclusion).strip()
        if term and term.casefold() in text_folded: blockers.append(f"exclusão da empresa encontrada: {term}")
    return blockers
...
def evaluate_row(row, profile=None):
    ...
    capability_evidence = capability_matches_text(capability, " ".join(str(v) for v in (lot.get("title"), lot.get("description")) if v))
    hard_blockers = _hard_capability_checks(lot, capability)
    result = match_lot(lot, profile)
    economics = evaluate_economic_fit(lot.get("value_numeric") or lot.get("value"), profile, opportunity=lot)
    ...
    reason = f"perfil={profile_score}; lote={lot_score}; geografia={geo['reason']}; capacidade={capability_evidence['reason']}; economic_fit={economics['status']}; {economics['reason']}"
    if hard_blockers: reason += "; bloqueios=" + ", ".join(hard_blockers)
    explanation = _explain_evaluation(profile_score, lot_score, geo, capability_evidence, economics, hard_blockers)
    return {
        "decision": decision, "reason": reason, "profile_score": profile_score, "lot_score": lot_score,
        "lot_id": result.get("lot_id"), "match": result.get("match"), "geography": geo,
        "commercial": result.get("commercial"), "economic_fit": economics, "capability": capability,
        "capability_evidence": capability_evidence, "hard_capability_blockers": hard_blockers,
...
def evaluate_and_record(conn, row, profile=None, account_id="default"):
    ...
    record_decision(conn, row.get("source") or "UNKNOWN", row.get("external_id") or str(row.get("id")), evaluation["decision"], evaluation["reason"], score=evaluation["profile_score"], features={
        "profile_score": evaluation["profile_score"], "lot_score": evaluation["lot_score"], "lot_id": evaluation["lot_id"],
        "match": evaluation["match"], "geography": evaluation["geography"], "commercial": evaluation["commercial"],
        "economic_fit": evaluation["economic_fit"], "capability_evidence": evaluation["capability_evidence"],
        "hard_capability_blockers": evaluation["hard_capability_blockers"], "explanation": evaluation["explanation"],
    }, rule_version=RULE_VERSION, account_id=account_id)
    return evaluation
