...
function describeNetworkError(error, path) {
  ...
  diagnostic.code = 'API_NETWORK_ERROR'; diagnostic.causeMessage = message;
  diagnostic.path = path; diagnostic.apiBase = API_BASE;
  return diagnostic;
}
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}${path}`, {
        ...
        headers: { Accept: 'application/json', ...contentHeaders, ...authHeaders, ...(fetchOptions.headers || {}) },
...
function normalizeOpportunity(item) {
  ...
  return { ...item, decision_score: item.decision_score ?? item.account_score ?? null, decision_reason: item.decision_reason ?? item.account_reason ?? null };
}
export const api = {
  ...
  billingStatus: () => request('/billing/status'), stats: () => request('/stats'), workflowStats: () => request('/workflow/stats'),
