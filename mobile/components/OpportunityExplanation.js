...
export default function OpportunityExplanation({ item }) {
  ...
  const factors = item?.decision_explanation;
  ...
  return <View style={styles.card}>
    ...
    <Factor
      title="Economic Fit"
      text={economics?.reason ? `${economics.status}: ${economics.reason}` : null}
      tone={economics?.status === 'UNFAVOURABLE' ? COLORS.red : COLORS.green}
    />

    {blockers.length ? <View style={styles.risk}>
      <Text style={styles.riskTitle}>Atenção</Text>
      {blockers.map((item, index) => <Text key={`${item}-${index}`} style={styles.riskText}>• {item}</Text>)}
    </View> : null}

    {factors?.reason ? <Text style={styles.technical}>{factors.reason}</Text> : null}
  </View>;
}
